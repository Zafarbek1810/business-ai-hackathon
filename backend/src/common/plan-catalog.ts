...
export const PRICING_PAGE_KEY = 'pricing_page';
...
export function sortPlanCatalog(items: PlanCatalogItem[]): PlanCatalogItem[] {
  const rank = new Map(PLAN_ORDER.map((id, index) => [id, index]));
  return [...items].sort(
...
export function buildPlanComparison(
  ...
  const valuesFor = (
    pick: (plan: PlanCatalogItem) => string | boolean | number,
