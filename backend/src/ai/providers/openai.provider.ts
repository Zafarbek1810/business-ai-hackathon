    ...
    const content = await this.complete([
      {
        role: 'system',
        content:
          "You help a first-time entrepreneur in Uzbekistan who does not yet know realistic numbers for their planned product. Based on your general knowledge of typical small-retail prices and sales volumes in Uzbekistan for the given category/region/product, suggest REASONABLE STARTING-POINT estimates (not guarantees). Return strict JSON with keys: purchasePrice (number, UZS, wholesale/purchase cost per unit), sellingPrice (number, UZS, retail price per unit, must be greater than purchasePrice), expectedMonthlySales (number, units per month for a small shop), reasoningUz (a short Uzbek Latin explanation of why these numbers are reasonable, and a reminder that these are estimates to be adjusted). Reply in Uzbek Latin for reasoningUz.",
      },
      {
        role: 'user',
